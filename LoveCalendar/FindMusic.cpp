...
//把文件信息连接到链表head中
int CFindMusics::saveToLink(struct fileInfoNode*& head,   //链表的头结点,引用参量
...
//显示整个查找到的文件的信息
void CFindMusics::displayLink(struct fileInfoNode* head)//IN:头结点，值传递参数
{
	while(head!=NULL)
	{
...
void CFindMusics::saveLinkToFile(struct fileInfoNode* head,string saveFileName,int counter)
{
	ofstream fout;
...
//
int CFindMusics::searchAllFile(string filePath,//IN:文件所在的路径,如：f:\example
